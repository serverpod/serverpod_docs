...
// TODO: These should link to the specific sections and the sections should have relatable names.
...
The extension of `SerializableEntity` makes the `Note` class capable of automatic serialization and deserialization. This makes the Note object transmittable between the server and the client.

// TODO: Is this a bit to simple in the explanation? Sounds like we are doing a sales pitch. Maybe remove some of it?
...
Serverpod provides built-in support for database integrations. By defining a database table named note, `table: note`, in the YAML file we easily create database bindings for our new `Note` class.
...
// TODO: DB_PASSWORD is used in the flowing text but POSTGRES_PASSWORD is used in the Mac and Linux example. What is right here?
```bash
...
:::info
// TODO: What happens to the data that already is stored in the database? Should we warn about the possible consequences?
...
// TODO: Is it important that Endpoint is a suffix of the endpoint? If so, please explain that it is and possibly why.
...
The first method parameter has to be a `Session` object. This is a special object in Serverpod that contains information about the current session, as well as other helpful methods.
...
// TODO: This is a bit unclear to me, what to we mean by following the same type restrictions? And the phrasing 'passed from the client' is also a bit unclear.
...
// TODO: Feels like some information is missing here. We should be explicit about where we define this method. Either in the code example below or in the text.
To store notes in the database we define a `createNote` method in the `NotesEndpoint` class. The method takes a `Note` object and stores it in the database. Observe that endpoint methods always need to take a `Session` parameter first.
...
In the above code, we use the `Note.insert` method, provided by the code generation, to insert the specified `Note` object into the database.
...
In the above code, we use the `Note.deleteRow` method, provided by the code generation, to delete the specified `Note` object from the database.
...
In the above code, we use the `Note.find` method, provided by the code generation, to retrieve all the notes from the database. By specifying `orderBy: Note.t.id`, we ensure that the notes are always returned in the same order based on the id column.
...
// TODO: Feels like we mix vocabulary here. Is Serverpod the 'project' or is the 'client', 'flutter' and 'server' different projects? I vote for saying that serverpod is the project and these are parts of it. Therefore the suggestion below.
...
Next, let's add a method for fetching notes from the server endpoint implemented earlier. The method updates the state with the notes if the request is successful, otherwise we call the `_connectionFailed` method to deal with the exception.
...
Finally, we want to fetch all notes when the app is initialized. We do this by overriding the `initState` method and call `_loadNotes` from there. 
...
// TODO: We name it function here, but earlier in the text we have named it method (and below the code example we again name it method). Are these free functions or are they methods on an object?
To create new notes we need to access the create note endpoint of our server. To do this we create a helper method called `_createNote` that takes a `Note` object as a parameter.
...
The `_createNote` method calls the `createNote` endpoint to store the note in the database on the server. Then `_loadNotes` is called to refresh the notes list.

// TODO: Users of flutter probably know this, but to make it easy to follow along in the guide and compare the code with out provided sample code - Where should this file be created?
...
We need a button to trigger this dialog and the floating action button is perfect for this. Add the following code to the build method inside the `MyHomePageState` class:
...
// TODO: Couldn't we provide an example that just shows the empty `Container()` replaced instead of having the user read through the full code of the app to understand where it should be replaced?
...
In this tutorial, you learned how to build a simple note-taking app using the Serverpod backend framework. You started by setting up the necessary tools and environment to work with Serverpod. Then, you covered various aspects of building an app, including creating serializable objects, creating a database tables, and creating API endpoints for performing CRUD operations.

You also learned how to use Serverpod's code generator and how to use the generated client library to connect the Flutter app to the server. By establishing this connection, you were able to fetch data from the server and display them in the app.
