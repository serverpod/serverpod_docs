...
Use the code generator to generate the code for the `Note` class from the definition in `note.yaml`. Run the following command from the root of your server project:
...
You need to replace `<DATABASE_PASSWORD>` with the password for the database, you can find it inside the `notes_server/config/passwords.yaml` file.
