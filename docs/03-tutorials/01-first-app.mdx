...
To define the structure of our Note object, we will create a YAML file called `note.yaml` inside `lib/src/model` folder in your Serverpod project (`notes_server`). Add the following content in `note.yaml`:
...
This creates a new migration that contains a description of the database schema and SQL code to add the table. These files can be found in the `generated/migration` directory.
