...
- **Simplified server initialization**: the generated `Serverpod` class comes pre-wired with your project's `Protocol` and `Endpoints`, so `server.dart` creates the server with just `Serverpod(args)`. The classic `Serverpod(args, Protocol(), Endpoints())` form keeps working.
