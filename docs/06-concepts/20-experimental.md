...
The Serverpod constructor now accepts the named argument `unstableDiagnosticEventHandlers` which is an optional array of diagnostic event handlers.
...
```dart
  var serverpod = Serverpod(
    ...
    unstableDiagnosticEventHandlers:[
      AsEventHandler((event, space, {required context}) {
...
is the new method `unstableSubmitDiagnosticEvent` on the Serverpod class.
