...
Alternatively, set the `SERVERPOD_PASSWORD_facebookAppId` and `SERVERPOD_PASSWORD_facebookAppSecret` [environment variables](../../../server-fundamentals/configuration#via-environment-variables) on your production server with the same values.
