...
| `--flutter` / `--no-flutter` | `true` | Auto-launch the companion Flutter apps marked `auto_launch: true` in the server's `pubspec.yaml`. Pass `--no-flutter` to disable auto-launch; apps can still be launched on demand from the terminal UI. |
...
:::note
Use `serverpod start` for local development only. For production, deploy the server instead. See [Deploy to Serverpod Cloud](../../../../08-deployments/01-deploy-to-serverpod-cloud.md) or [Custom hosting](../../../../08-deployments/custom-hosting/01-choosing-a-strategy.md).
:::
