...
:::info
For a method to be recognized by Serverpod as a future call, it must return a `Future<void>` and have at least one positional parameter which must be a [`Session`](../endpoints-and-apis/sessions) object. You can pass any serializable types as other parameters, and even use `List`, `Map`, `Set` or Dart records as long as they are typed. `Streaming` parameters are not supported.
:::
