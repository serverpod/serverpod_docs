...
```bash
docker cp tables-auth.pgsql <container_name>:/docker-entrypoint-initdb.d/tables-auth.pgsql

docker exec -u postgres <container_name> psql <db_name> postgres -f docker-entrypoint-initdb.d/tables-auth.pgsql
```

**<container_name>** is normally `project_name_server-postgres-1` but you can find the exact name by running `docker ps`.
