...
To define the structure of our Note object, we will create a YAML file called `note.spy.yaml` inside the `lib/src/models` directory in your Serverpod project (`notes_server`). Add the following content in `note.spy.yaml`:
...
Let's take a closer look at the content of the `note.spy.yaml` file:
...
Use the code generator to generate the code for the `Note` class from the definition in `note.spy.yaml`. Run the following command from the root of your server project (`notes_server`):
...
The updated content of the `note.spy.yaml` file should look like this:
