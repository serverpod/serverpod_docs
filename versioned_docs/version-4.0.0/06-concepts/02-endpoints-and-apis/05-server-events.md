...
Serverpod uses Redis to pass messages between servers. A message is sent to every server connected to the same Redis instance when [Redis is enabled](../operations/redis), and stays on the current server when it is not. Set the `scope` parameter to choose the delivery explicitly.
