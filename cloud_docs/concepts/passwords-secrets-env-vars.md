...
One of those Auth passwords is `scloudAuthEmailKey`, which new projects with authentication enabled use to send registration and password-reset codes through Cloud's email service. It is provisioned on deploy, so email sign-in works without any setup on your side. See [Set up email sign-in](/concepts/authentication/providers/email/setup) for the run-mode behavior, what happens when delivery fails, and how to send the codes through your own email provider instead.
